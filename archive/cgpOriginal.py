...
class Function:
    ...
    def __init__(self, f, arity, name=None):
        # The function
        self.f = f
        # The arity, i.e., number of inputs
        self.arity = arity
...
class Individual:
    ...
    n_outputs = 1
    n_cols = N_COLS
    # On mine, the number of columns should be the same as the levels back (according to PP)
    ...
    def __init__(self):
        ...
        for pos in range(self.n_cols):
            # Create a random node, passing its index in the list (column number) as a parameter
            self.nodes.append(self._create_random_node(pos))
        # Set only the very last node (output) to active - can be increased for multiple outputs?
    ...
    def _create_random_node(self, pos):
        ...
        # Choose the input function randomly
        node.i_func = random.randint(0, len(self.function_set) - 1)
        # For each input available for the function (typically two)
        for i in range(self.function_set[node.i_func].arity):
            # Randomly set the input to be one of the preceding nodes within the level back value
            node.i_inputs[i] = random.randint(max(pos - self.level_back, -self.n_inputs), pos - 1)
            # Randomly set the weight based on the weight range
    ...
    def _determine_active_nodes(self):
        ...
        n_active = 0
        # Reverse the list and check every node
        for node in reversed(self.nodes):
            if node.active:
                n_active += 1
                # For the number of inputs into the node
                for i in range(self.function_set[node.i_func].arity):
        ...
        for node in self.nodes:
            if node.active:
                ...
                for i in range(self.function_set[node.i_func].arity):
                    ...
                    w = node.weights[i]
                    if i_input < 0:
                        # Append one of the input parameters provided as a list to the args parameter and weigh it
                        # Generalized, but for flappy bird it receives [v, h, g] so args[-(-2)-1] = args[1] = h
